...
def deduplicated_density(k, m):
    ...
    for n in range(1, V + 1):
        ...
        for r in range(1, U + 1):
            val += (
                Decimal(1)
                - (Decimal(r) / Decimal(U)) ** w
                + (Decimal(r - 1) / Decimal(U)) ** w
            ) ** Decimal(n)
        res += (Decimal(U) - val) / Decimal(n) * Decimal(comb(V, n, exact=True))
...
plt.show()
fig.tight_layout()
fig.savefig("deduplicated_density_random_minimizers_k8.pdf")
